...
export default function ViewportSlot() {
  ...
  const equipColor = { RUNNING: '#34d399', IDLE: '#9aa3b2', QA_ALERT: '#facc15',
    MODEL_TRAINING: '#a78bfa', THERMAL_FAULT: '#f87171' }[line?.equipment_status] || '#9aa3b2'
  const gpuColor = { cool: '#1FB8CD', warm: '#facc15', hot: '#fb923c', critical: '#f87171' }[telemetry?.thermal] || '#9aa3b2'
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative' }}>
      ...
      <div style={{ position: 'absolute', top: 10, left: 12, display: 'flex', gap: 5, flexWrap: 'wrap' }}>
        ...
        {line && (
          <span style={{ ...chip, color: equipColor }}>
            {line.equipment_status} · {line.conveyor_speed_mps}m/s
            {line.tact_time_s != null ? ` · tact ${line.tact_time_s}s` : ''}
          </span>
        )}
        {telemetry?.has_gpu && (
          <span style={{ ...chip, color: gpuColor }}>
            GPU {telemetry.temp_c}°C · VRAM {telemetry.vram_pct}%
            {telemetry.training ? ' · 학습 중' : ''}
          </span>
        )}
      </div>
    </div>
