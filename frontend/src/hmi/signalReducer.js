...
export const initialState = {
  ...
  // ── 공장 트윈 라인(백엔드 FactoryLine, type='line' 2s 주기) ──
  line: null,                  // {conveyor_speed_mps, tact_time_s, throughput_per_min, uptime_s, equipment_status, ...}
  lineStats: null,             // {total, ok, ng, defect_rate, line_status, ...}
  telemetry: null,             // GPU 요약 {thermal, load, temp_c, vram_pct, util_pct, training, ...}
  telemetryFull: null,         // 풀 텔레메트리(type='telemetry' 6s 주기) — GPU별 상세
...
export function applyMessage(state, msg) {
  ...
  switch (t) {
    ...
    case 'line': {
      // 공장 라인 이벤트 — 설비 상태 악화 전이 시에만 알람(경보 피로 방지)
      const patch = { line: msg.line || null, lineStats: msg.stats || null }
      if (msg.telemetry) patch.telemetry = msg.telemetry
      const prev = state.line?.equipment_status
      const cur = msg.line?.equipment_status
      if (cur && cur !== prev && (cur === 'THERMAL_FAULT' || cur === 'QA_ALERT')) {
        const text = cur === 'THERMAL_FAULT'
          ? `GPU 발열 위험 — 라인 감속 (온도 ${msg.telemetry?.temp_c ?? '?'}°C)`
          : `불량률 ${((msg.stats?.defect_rate ?? 0) * 100).toFixed(0)}% > 목표 ${((msg.stats?.defect_target ?? 0) * 100).toFixed(0)}%`
        patch.alarms = [{ ts: Date.now() / 1000, level: cur === 'THERMAL_FAULT' ? 'error' : 'warn',
          tag: cur, text }, ...state.alarms].slice(0, 100)
      }
      return patch
    }

    case 'telemetry':
      return { telemetryFull: msg, telemetry: msg.summary || state.telemetry }
