...
class Engine:
    ...
    def _admit(self, n, part, previous=None):
        ...
        j = dict(part=part, entered=self.now, phase=phase, due=self.now + delay, duration=duration)
        n['jobs'].append(j)
        if service:
            self.schedule(j['due'], 'complete', c.id, part['id'], n['epoch'])
    ...
    def _event(self):
        ...
        if kind == 'arrival':
        ...
        elif kind == 'failure' and not c.maintenance:
            n['down'] = True
            n['epoch'] += 1
            for j in n['jobs']:
                if j['phase'] != 'ready':
            ...
            for j in n['jobs']:
                if j['phase'] != 'ready':
                    j['due'] = self.now + j.pop('remaining', 0.)
                    self.schedule(j['due'], 'complete', cid, j['part']['id'], n['epoch'])
            ...
            if j:
                if j['phase'] == 'setup':
                    ...
                    self.schedule(j['due'], 'complete', cid, pid, n['epoch'])
                else:
                    j['phase'] = 'ready'
    ...
    def metrics(self):
        ...
        for cid, n in self.nodes.items():
            ...
            resources[cid] = dict(kind=c.kind, state=n['state'], occupancy=len(n['jobs']), capacity=c.capacity,
    ...
    def snapshot(self):
        ...
        for cid, n in self.nodes.items():
            for j in n['jobs']:
                parts.append(dict(**j['part'], component=cid, phase=j['phase'], entered=j['entered'], due=j['due']))
        return dict(model=self.model.model_dump(), status=self.status, time=self.now, error=self.error,
