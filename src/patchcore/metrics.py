...
def compute_imagewise_retrieval_metrics(
    ...
    """
    Computes retrieval statistics (AUROC, FPR, TPR, Precision, Recall, F1).
    ...
    # Compute best-F1 threshold Precision/Recall/F1
    prec_arr, rec_arr, pr_thresholds = metrics.precision_recall_curve(
        anomaly_ground_truth_labels, anomaly_prediction_weights
    )
    f1_arr = np.where(
    ...
    best_precision = float(prec_arr[best_pr_idx])
    best_recall    = float(rec_arr[best_pr_idx])
    f1             = float(f1_arr[best_pr_idx])
