...
class PatchCore(torch.nn.Module):
    ...
    def _fill_memory_bank(self, input_data):
        ...
        # ── MBQS: log memory bank quality score (ClassConditionedIrredundantSampler only) ──
        self.last_mbqs = None
        if hasattr(self.featuresampler, 'evaluate_memory_quality'):
            try:
                mem_tensor = torch.from_numpy(features).float()
                # For MBQS, use the sampled memory bank as both memory and proxy for full features
                # (full features are not retained post-sampling; use memory bank itself)
                mbqs = self.featuresampler.evaluate_memory_quality(
                    memory=mem_tensor,
                    full_features=mem_tensor,
                )
                self.last_mbqs = mbqs
                LOGGER.info(f"[MBQS] score={mbqs:.4f} (class={getattr(self.featuresampler, 'class_name', 'unknown')})")
            except Exception as _e:
                LOGGER.warning(f"[MBQS] Could not compute quality score: {_e}")
