...
#
#   backend  → uvicorn server.app:app (P-core: REST API + WebSocket + HMI 서빙)
...
# 기본 실행: P-core (docker-compose에서 ml_worker는 command를 오버라이드)
CMD ["uvicorn", "server.app:app", "--host", "0.0.0.0", "--port", "8200"]
